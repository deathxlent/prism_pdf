#!/bin/bash
# ============================================================
# Prism PDF - 一键启动脚本 (macOS Apple Silicon)
# 同时启动 llama.cpp OCR 服务和主服务
# 仅支持 M1/M2/M3/M4 系列芯片
# ============================================================
...
echo ""
echo -e "${CYAN}============================================${NC}"
echo -e "${CYAN}  Prism PDF - 一键启动 (macOS Apple Silicon)${NC}"
echo -e "${CYAN}============================================${NC}"
echo ""
...
if [ "$SKIP_OCR" = false ]; then
    echo -e "${YELLOW}[1/2] 启动 llama.cpp OCR 服务 (Metal 加速)...${NC}"
...
echo "        - 加速方式: Apple Silicon MPS + Metal"
echo ""
